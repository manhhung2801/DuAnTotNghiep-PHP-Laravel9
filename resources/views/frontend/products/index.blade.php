...
@section('title', ($category->name ?? ($subCategory->name ?? ($childCategory->name ?? ($product->name ?? ' Sản phẩm')))) . ' Chính hãng giá rẻ, ưu đãi, trả góp 0%')
@section('description', 'Mua ngay ' . ($category->name ?? ($subCategory->name ?? ($childCategory->name ?? ($product->name ?? 'sản phẩm ')))) . ' với mức giá ưu đãi, bảo hành uy tín kèm nhiều chương trình khuyến mãi giảm giá lớn, dịch vụ chăm sóc')
@section('schema')
{{-- WebSite --}}
...
{{-- Product --}}
<script type="application/ld+json">{ "@context": "https://schema.org", "@type": "Product", "name": {{ $category->name ?? ($subCategory->name ?? ($childCategory->name ?? ($product->name ?? ''))) }}, "image": [ {{ asset('uploads/logo/cybermart_logo_1x1.png') }} ], "description": "Mua .{{ ($category->name ?? ($subCategory->name ?? ($childCategory->name ?? ($product->name ?? 'sản phẩm ')))) }}. - Apple chính hãng, giá rẻ. Hư gì đổi nấy 12 tháng, thu cũ Đổi mới giảm đến 2tr, có trả góp 0%. BH chính hãng 1 năm, freeship lần mua đầu. Xem ngay!", "brand": { "@type": "Brand", "name": {{ ($category->name ?? ($subCategory->name ?? ($childCategory->name ?? ($product->name ?? 'sản phẩm ')))) }} }, "offers": { "@type": "AggregateOffer", "priceCurrency": "VND", "seller": { "@type": "organization", "@id": "{{ url('/') }}#organization", "name": "CyberMaer" }, "offerCount": 10, "lowPrice": 8990000, "highPrice": 29390000 } }</script>
{{-- organization --}}
<script type="application/ld+json">{ "@context": "https://schema.org", "@type": "organization", "@id": "{{ url('/') }}#organization", "name": "Thương Mại Điện Tử CyberMart", "url": "{{ url('/') }}", "logo": { "@type": "ImageObject", "@id": "{{ url('/') }}#logo", "url": "{{ url('/') }}/uploads/logo/cybermart_logo_1x1.png", "contentUrl": "{{ url('/') }}/uploads/logo/cybermart_logo_1x1.png", "width": 250, "height": 250, "caption": "Thương Mại Điện Tử CyberMart" } }</script>
@endsection
